...
# 귀무 가설 : 기대치와 관찰치는 차이가 없다. 현재 주사위는 게임에 적합하다.
# 대립 가설 : 기대치와 관찰치는 차이가 있다. 현재 주사위는 게임에 적합하지 않다. (펼치고 싶은 주장)
...
# 판정 : p-value : p-value:0.01439 < 0.05 이므로 귀무 기각
# 기대치와 관찰치는 차이가 있다. 현재 주사위는 게임에 적합하지 않다. 현재 관찰된 데이터는 우연히 발생한 데이터가 아니다.

# cv로 판정 df:? -> (N-1) = 5  /  cv:? -> 11.07  / chi2 : 14.200    => 귀무 기각역에 존재하네
...
print(stats.chisquare(datas.관측도수)) # statistic=20.488188976377952, pvalue=0.000399
# pvalue=0.000399 < 0.05 이므로 귀무기각 -> 스포츠 음료에 대한 선호도에 차이가 있다. 그러므로 특정음료 제공을 더 많이 할수 있도록 한다.
